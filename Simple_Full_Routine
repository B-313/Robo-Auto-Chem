...
def basic_recorder(vial_number, camera_index=0, width=640, height=480, fps=30, record_seconds=10):
    base_dir   = "/home/robot/group_B/robo_chem_504/group_B_videos"
    video_path = os.path.join(base_dir, f"{vial_number + 1}.mp4")
    csv_path   = os.path.join(base_dir, f"{vial_number + 1}_log.csv")
    ...
    while True:
        ...
        ts = (recording_start + timedelta(seconds=elapsed)).strftime("%H:%M:%S.%f")[:-3]
        log.append([ts, elapsed, frame_count])
    ...
    # Write per-frame log to CSV
    with open(csv_path, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['timestamp', 'elapsed_s', 'frame'])
        writer.writerows(log)

    print(f"Video saved to: {video_path}")
    print(f"Log  saved to: {csv_path}  ({len(log)} frames)")
